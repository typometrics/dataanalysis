...
\block{}{
  ...
  \begin{minipage}[c]{0.07\linewidth}\centering
    ...
    \parbox{4.8cm}{%
      \tabhead{%
      \resizebox{4.8cm}{!}{YOUR}\\[0.5mm]
      \resizebox{4.8cm}{!}{LANGUAGE}\\[0.5mm]
      \resizebox{4.8cm}{!}{SECRETS}\\[0.5mm]
      \resizebox{4.8cm}{!}{ARE HERE}}}}
  \end{minipage}\hfill
  % --- Fat arrow ---
  \begin{minipage}[c]{0.02\linewidth}\centering
    \textcolor{tabred}{%
    \begin{tikzpicture}
      \draw[-{Triangle[length=18pt,width=20pt]}, line width=6pt, tabred]
        (0,0) -- (1.8,0);
    \end{tikzpicture}}
  \end{minipage}\hfill
  % --- QR + URL ---
  \begin{minipage}[c]{0.16\linewidth}
    \begin{minipage}[c]{0.48\linewidth}\centering
      \qrcode[height=6cm, level=H]{https://typometrics.elizia.net/menzerath}
    \end{minipage}\hfill
    \begin{minipage}[c]{0.48\linewidth}\raggedright
      {\fontsize{28}{32}\selectfont \textbf{typometrics.elizia.net\slash menzerath}}\par\vspace{2mm}
...
\block{%
  ...
  {\huge \textcolor{tabred}{\textbf{NEW!}}\ Our data extraction is filtered: we exclude {\large\texttt{punct, discourse, parataxis, conj, cc, vocative, aux, compound:prt, mark, case}} from the verbal construction. Only \textbf{real syntactic constituents} are retained.}\par\vspace{5mm}
  ...
  \begin{center}
  \includegraphics[width=0.85\linewidth]{vogel.png}
  \end{center}
  \vspace{2mm}
...
\block{%
  ...
  \begin{minipage}[t]{0.48\linewidth}
    ...
    {\huge We compute three variants of the MAL effect. $\text{MAL}_n(L)$ = mean size of constituents in all verbal constructions of L with $n$ constituents (left + right combined). Standard MAL as in the literature.}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.48\linewidth}
  ...
  {\huge \textbf{MAL effect} $= \beta$, the slope of the regression line in log-log space.}\par\vspace{4mm}
  {\huge $\beta > 0.1$: \textbf{MAL language}. \quad $\beta < -0.1$: \textbf{Anti-MAL}. \quad In between: \textbf{grey zone}.}\par\vspace{8mm}
...
% --- Small world map below the definitions ---
\block{}{%
  \raggedright
  \begin{minipage}[c]{0.17\linewidth}\raggedright
    \tabkicker{Menzerath is everywhere}
    {\fontsize{38}{44}\selectfont\tabhead{MAL Effect}}\par\vspace{3mm}
    \rule{\linewidth}{3pt}\par\vspace{3mm}
    {\huge  $-\beta(1{\to}\text{max})$ \\ 171 languages}
  \end{minipage}\hfill
  \begin{minipage}[c]{0.81\linewidth}
    \includegraphics[width=\linewidth]{../plots/poster_world_map_v4}
  \end{minipage}
}
\useblockstyle{tabloid}
\end{columns}
...
\block{%
  ...
  {\fontsize{78}{86}\selectfont\tabhead{German nails it. Naija wobbles. Arabic surprises.}}}{
  \raggedright
  \begin{minipage}[t]{0.31\linewidth}\raggedright
    ...
    {\huge \textbf{slope = --0.19, R\textsuperscript{2} \textasciitilde\ 0.89}
  ...
  \begin{minipage}[t]{0.31\linewidth}\raggedright
    ...
    \textcolor{tabred}{\textbf{And you?} Check your language at \textbf{typometrics.elizia.net/menzerath}!}}
  \end{minipage}\par
...
\block{%
  ...
  \begin{minipage}[t]{0.30\linewidth}\raggedright
    \vspace{0pt}
    {\centering\includegraphics[width=\linewidth]{../plots/poster_strip_mal_v4}\par}
    \vspace{3mm}
    {\fontsize{38}{44}\selectfont\tabhead{The Word Order Illusion}}\par\vspace{3mm}
    {\huge They want you to think word order is irrelevant. Look at the global averages: MAL and Anti-MAL types are distributed with eerie equality across VO and OV languages. It's the perfect cover-up. But once you split the verb in half, their entire symmetry falls apart...}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.30\linewidth}\raggedright
    \vspace{0pt}
    {\centering\includegraphics[width=\linewidth]{../plots/poster_strip_rmal_v4}\par}
    \vspace{3mm}
  ...
  \begin{minipage}[t]{0.30\linewidth}\raggedright
    \vspace{0pt}
    {\centering\includegraphics[width=\linewidth]{../plots/poster_strip_lmal_v4}\par}
    \vspace{3mm}
