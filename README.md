...
**`verb_centered_analysis.py`** - Verb-Centered Tables
- `compute_average_sizes_table()` - Aggregates position-wise averages
- `format_verb_centered_table()` - ASCII table formatting with optional growth factors
...
- Original fast validation functions
- `getAllConllFiles()` - File discovery
- `checkLangCode()` / `checkLangGroups()` - Validation helpers

### Debugging & Testing

**`debug_bastard_examples.py`** - Bastard dependency tests
**`debug_exact.py`** - Exact dependency size verification
**`debug_user_example.py`** - User-reported issue reproduction
**`test_dep_sizes_bastards.py`** - Unit tests for bastard statistics
**`reproduce_bastards.py`** - Bastard analysis reproduction

### Unused/Legacy Files

Based on analysis of imports and references:

**Potentially Unused Python Modules:**
- `find_de_files.py` - German file finder (not imported by any notebook)
- `manage.py` - Django management (Django not present in this workspace)
- `test_plot.py` - Test plotting script (standalone, not in pipeline)
- `06_factor_analysis.py` - Duplicate of notebook 06 (standalone version)

**Note**: These files may be for standalone execution or legacy purposes
