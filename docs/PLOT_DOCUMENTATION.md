...
To prevent overwrites and ensure clarity, filenames follow specific patterns:

**Prefixes:**
- `scatter_2d_*`: Standard 2D scatter plots.
- `scatter_3d_*`: 3D scatter plots.
- `mal_curves_*`: Mean Average Length curves.
- `wals_validation_*`: Validation against WALS data.
- `position_distribution_*`: Bar charts of frequent items at positions.

**Grouping Suffixes:**
- `*_by_group.png`: Colored by Language Family/Group (e.g., IE, Afro-Asiatic).
- `*_by_Label.png`: Colored by specific labels.
...
*   **File**: `position_distribution_{side}-{top_n}.png` (e.g., `position_distribution_left-30.png`)
*   **Description**: Bar charts showing the most frequent dependency types or POS tags at specific positions (e.g., L1, R1).
*   **Usage**: Used to understand typical occupants of syntactic slots.

### 3D Analysis
*   **File**: `scatter_3d_{z}_vs_{y}_vs_{x}_by_{group}.png`
*   **Example**: `scatter_3d_MAL3_vs_MAL2_vs_MAL1_by_Family.png`
*   **Description**: 3D scatter plot visualizing relationships between three metrics simultaneously (often MAL at n=1, 2, 3).
...
## Diagonal Analysis (Size vs. Factor)

### Pattern: `DIAG_{side}_{positions}.png` and `*_extreme_diag_*.png`

**Generated by**: Verb-centered helix tables with diagonal factors enabled

**Description**: It is crucial to distinguish between "Size" plots and "Factor" plots, which look similar but measure different things.

### 1. Diagonal Growth Factors
*   **File Pattern**: `*_factor_vs_*.png`, `right_extreme_diag_factor_vs_vo.png`
*   **Definition**: The **Geometric Mean of Growth Factors** along a diagonal (e.g., R2/R1, R3/R2).
*   **Meaning**: Represents the **rate of growth/expansion** of constituents along the diagonal.
*   **Interpretation**:
    - Factor > 1: Size increases along diagonal trajectory.
    - Factor < 1: Size decreases (compression).

### 2. Diagonal Sizes
*   **File Pattern**: `right_extreme_diag_vs_vo.png` (no "factor" in name)
*   **Definition**: The **Geometric Mean of Sizes** along the longest diagonal (positions R1, R2, R3, R4).
*   **Meaning**: Represents the **absolute magnitude** of constituent weights along the diagonal.
...
### Notebook 03: Visualization & Exploration
- Generates `position_distribution` and `scatter_3d` plots.
- Focuses on initial data exploration and distributions.
